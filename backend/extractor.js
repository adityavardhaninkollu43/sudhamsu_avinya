...
Return ONLY a valid JSON object matching this schema:
{
  ...
  "parameters": [
    ...
    // Use high-availability models with gemini-3.1-flash-lite as primary
    const candidateModels = ['gemini-3.1-flash-lite', 'gemini-3.6-flash', 'gemini-flash-latest', 'gemini-3.8-flash', 'gemini-3.1-pro-preview'];
    ...
    for (const modelName of candidateModels) {
      ...
      while (attempts < 2) {
        try {
          ...
          attempts++;
          const is503 = err.message && (err.message.includes('503') || err.message.includes('demand'));
          if (is503 && attempts < 2) {
            console.warn(`[Extractor] Model ${modelName} hit 503 spike, backing off 600ms before retry...`);
    ...
    const cleanJson = geminiResponse.text.replace(/```json\s*/gi, '').replace(/```\s*$/g, '').trim();
