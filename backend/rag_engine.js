...
`;
        ...
        const ragModels = ['gemini-3.1-flash-lite', 'gemini-3.6-flash', 'gemini-3.8-flash'];
        let response = null;
        let selectedModel = 'gemini-3.1-flash-lite';

        for (const m of ragModels) {
          try {
            ...
            console.warn(`[RAG] Model ${m} unavailable: ${mErr.message}. Trying next model...`);
          }
        }

        if (response && response.text) {
          answerText = response.text;
          modelUsed = `GEMINI_GROUNDED_RAG (${selectedModel})`;
          sources.push({ type: 'llm_grounded_inference', model: selectedModel });
