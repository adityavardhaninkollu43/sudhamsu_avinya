...
let flaggedP = PARAMETERS.filter((p) => statusOf(p) === "out");
let inRangeP = PARAMETERS.filter((p) => statusOf(p) === "in");
let find = (id) => PARAMETERS.find((p) => p.id === id || p.canonicalId === id);
...
// Pre-fill fasting and medication context from Smart Contextualizer if available
try {
  const contextMeds = Store.get("clinical_active_meds", []);
  const fastingState = Store.get("clinical_fasting_state", null);
  if (!saved.fast && fastingState) {
    $("#fast").value = fastingState === "fasting" ? "Fasting (overnight 10-12 hours, water only)" : "Non-fasting (recent meal/beverage consumption)";
  }
  if (!saved.meds && Array.isArray(contextMeds) && contextMeds.length) {
    const medLabels = {
      metformin: "Metformin (oral antihyperglycemic)",
      statin: "Atorvastatin / Statin (lipid-lowering therapy)",
      biotin: "Biotin high-dose supplement (known immunoassay interferent)",
      levothyroxine: "Levothyroxine (thyroid hormone replacement)",
      nsaid: "NSAID / Ibuprofen (regular analgesia)",
      ace_arb: "ACE-Inhibitor / ARB (antihypertensive)"
    };
    $("#meds").value = contextMeds.map(m => medLabels[m] || m).join("; ");
  }
  const userConditions = Store.get("user_health_conditions", []);
  if (!saved.context && Array.isArray(userConditions) && userConditions.length) {
    const conditionLabels = {
      hypertension: "High Blood Pressure / Hypertension",
      diabetes: "Diabetes / Elevated Blood Sugar",
      cholesterol: "High Cholesterol / Dyslipidemia",
      fatty_liver: "Fatty Liver Disease (NAFLD)",
      uric_acid: "High Uric Acid / Gout",
      kidney: "Kidney Health / Reduced eGFR",
      thyroid: "Thyroid Condition",
      gerd: "Acid Reflux / GERD"
    };
    $("#context").value = "Reported conditions: " + userConditions.map(c => conditionLabels[c] || c).join(", ");
  }
} catch (e) {
  console.warn("Context prefill:", e);
}
  ...
  $("#sheet").innerHTML = `
    ...
    <section>
      ...
      <table>
        ...
        <tr><td style="color:var(--ink-3)">Laboratory</td><td>${esc(REPORT.lab || REPORT.laboratory || "Clinical Diagnostic Laboratory")}</td></tr>
...
function asText() {
  ...
  out.push(line("Laboratory", REPORT.lab || REPORT.laboratory || "Clinical Diagnostic Laboratory"));
  ...
  if (targetReportId) {
    try {
      ...
      if (res && res.data && res.data.parameters && res.data.parameters.length) {
        ...
        flaggedP = PARAMETERS.filter((p) => statusOf(p) === "out");
        inRangeP = PARAMETERS.filter((p) => statusOf(p) === "in");
        find = (id) => PARAMETERS.find((p) => p.id === id || p.canonicalId === id);
        if (!saved.who && REPORT.patient && REPORT.patient.name) {
          $("#who").value = REPORT.patient.name;
        }
        if (!saved.dob && REPORT.patient && REPORT.patient.age) {
          $("#dob").value = REPORT.patient.age;
        }
        drawGate();
        draw();
      }
    } catch (e) {
      console.warn("Could not load report for summary:", e);
    }
  }

  // Populate report switcher
  const switcher = $("#summary-report-switcher");
  if (switcher) {
    try {
      ...
      if (listRes && listRes.data && listRes.data.length) {
        ...
        switcher.onchange = (e) => {
          ...
          location.href = `summary.html?report=${encodeURIComponent(e.target.value)}`;
        };
      } else {
