import express from 'express';
import path from 'path';
import fs from 'fs';
import { fileURLToPath } from 'url';
import multer from 'multer';
import jwt from 'jsonwebtoken';
import bcrypt from 'bcryptjs';
import { ragEngine } from './backend/rag_engine.js';
import { extractLabReport } from './backend/extractor.js';
import { generateClinicalReportPdf } from './backend/pdf_generator.js';
import { createReadoutDeck } from './backend/make_presentation.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const app = express();
const PORT = 3000;
const JWT_SECRET = process.env.JWT_SECRET || 'readout_medical_jwt_secret_dev_key_change_in_prod';

// Configure multer for file uploads
const uploadDir = path.join(__dirname, 'uploads');
if (!fs.existsSync(uploadDir)) {
  fs.mkdirSync(uploadDir, { recursive: true });
}
const storage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, uploadDir),
  filename: (req, file, cb) => {
    const ext = path.extname(file.originalname);
    const safeName = `${Date.now()}-${Math.random().toString(36).substring(2, 8)}${ext}`;
    cb(null, safeName);
  }
});
const upload = multer({
  storage,
  limits: { fileSize: 20 * 1024 * 1024 }, // 20MB
  fileFilter: (req, file, cb) => {
    const allowed = /\.(pdf|jpg|jpeg|png|webp|heic)$/i.test(file.originalname);
    if (!allowed) {
      return cb(new Error('Unsupported file format. Please upload a PDF, PNG, JPG, or WEBP file.'));
    }
    cb(null, true);
  }
});

app.use(express.json());
app.use(express.urlencoded({ extended: true }));

// Safety Headers Middleware
app.use((req, res, next) => {
  res.setHeader('X-Medical-Safety-Disclaimer', 'Readout flags results; it does not diagnose, treat, or replace a clinician.');
  res.setHeader('X-Content-Type-Options', 'nosniff');
  next();
});

// Health check endpoints
app.get(['/health', '/api/health'], (req, res) => {
  res.json({
    status: 'healthy',
    version: '2.0.0',
    timestamp: new Date().toISOString(),
    services: {
      ocr_pipeline: 'ready',
      clinical_decision_support: 'active',
      audit_logging: 'active',
      storage: 'online'
    },
    safety_disclaimer: 'Readout flags results against reference ranges; it does not diagnose, treat, or replace a licensed clinician.'
  });
});

// In-Memory Database with Initial Seed Data
const USERS = new Map([
  [
    'demo@readout.health',
    {
      id: 'usr-demo-001',
      email: 'demo@readout.health',
      passwordHash: bcrypt.hashSync('demopassword123', 8),
      fullName: 'Clinical Demo Patient',
      createdAt: new Date().toISOString()
    }
  ]
]);

const AUDIT_LOGS = [
  {
    id: 'aud-001',
    timestamp: new Date().toISOString(),
    action: 'SYSTEM_BOOT',
    resourceType: 'SERVER',
    details: { message: 'Readout Clinical Engine booted' }
  }
];

function logAudit(userId, action, resourceType, details) {
  const record = {
    id: `aud-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
    timestamp: new Date().toISOString(),
    userId: userId || 'anonymous',
    action,
    resourceType,
    details
  };
  AUDIT_LOGS.unshift(record);
  if (AUDIT_LOGS.length > 500) AUDIT_LOGS.pop();
  return record;
}

// Default Seed Parameters
const SEED_PARAMETERS = [
  {
    id: "hb",
    name: "Haemoglobin",
    abbr: "Hb",
    group: "Blood count",
    value: 13.4, unit: "g/dL", low: 12.0, high: 15.5,
    what: "Haemoglobin is the protein inside red blood cells that carries oxygen from your lungs to the rest of your body.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Haemoglobin moves with iron levels, recent blood loss, pregnancy, hydration and altitude.",
    ask: "",
    terms: "Reported as HGB or Hb. Measured in grams per decilitre (g/dL).",
    isVerified: true
  },
  {
    id: "hct",
    name: "Haematocrit",
    abbr: "HCT",
    group: "Blood count",
    value: 40.2, unit: "%", low: 36, high: 46,
    what: "Haematocrit is the share of your blood made up of red blood cells, given as a percentage.",
    reading: "This value sits inside the range printed on your report.",
    influences: "It tracks closely with haemoglobin and rises temporarily when you are dehydrated.",
    ask: "",
    terms: "Reported as HCT or PCV (packed cell volume).",
    isVerified: true
  },
  {
    id: "wbc",
    name: "White blood cell count",
    abbr: "WBC",
    group: "Blood count",
    value: 11.8, unit: "×10³/µL", low: 4.0, high: 11.0,
    what: "White blood cells are the cells your immune system uses to respond to infection and injury.",
    reading: "This value is above the upper end of the range printed on your report.",
    influences: "White cell counts commonly rise during and shortly after an infection, with physical stress, after strenuous exercise, in smokers, and with some medicines such as steroids.",
    ask: "Were you unwell, or recovering from anything, around the day of this test?",
    terms: "Reported as WBC, TLC or total leucocyte count.",
    isVerified: true
  },
  {
    id: "plt",
    name: "Platelet count",
    abbr: "PLT",
    group: "Blood count",
    value: 268, unit: "×10³/µL", low: 150, high: 410,
    what: "Platelets are small cell fragments that help your blood clot when you are cut or bruised.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Counts shift with infection, inflammation, iron levels and some medicines.",
    ask: "",
    terms: "Reported as PLT or platelet count.",
    isVerified: true
  },
  {
    id: "glucose",
    name: "Fasting blood glucose",
    abbr: "FBS",
    group: "Blood sugar",
    value: 112, unit: "mg/dL", low: 70, high: 99,
    what: "This measures the amount of sugar in your blood after a period without food, usually eight to twelve hours.",
    reading: "This value is above the upper end of the range printed on your report.",
    influences: "Fasting glucose is affected by how long you actually fasted, illness, poor sleep, stress, physical activity and several medicines including steroids.",
    ask: "How many hours had I fasted, and does that change how you read this number?",
    terms: "Reported as FBS, FPG or fasting plasma glucose.",
    isVerified: true
  },
  {
    id: "hba1c",
    name: "Glycated haemoglobin",
    abbr: "HbA1c",
    group: "Blood sugar",
    value: 6.1, unit: "%", low: 4.0, high: 5.6,
    what: "HbA1c reflects your average blood sugar over roughly the previous two to three months, rather than on the morning of the test.",
    reading: "This value is above the upper end of the range printed on your report.",
    influences: "Because it depends on red blood cells, HbA1c can read differently in people with anaemia, recent blood loss, or certain haemoglobin variants.",
    ask: "Does my haemoglobin level affect how I should read this HbA1c?",
    terms: "Reported as HbA1c or A1C.",
    isVerified: true
  },
  {
    id: "chol",
    name: "Total cholesterol",
    abbr: "CHOL",
    group: "Lipids",
    value: 214, unit: "mg/dL", low: null, high: 200,
    what: "Total cholesterol adds together the different cholesterol-carrying particles in your blood.",
    reading: "This value is above the upper limit printed on your report.",
    influences: "Diet, physical activity, weight, thyroid function, family history and some medicines all move cholesterol numbers.",
    ask: "Which of these lipid numbers matters most in my case?",
    terms: "Reported as TC or total cholesterol.",
    isVerified: true
  },
  {
    id: "ldl",
    name: "LDL cholesterol",
    abbr: "LDL-C",
    group: "Lipids",
    value: 141, unit: "mg/dL", low: null, high: 100,
    what: "LDL carries cholesterol from the liver out to the body. Guidelines set a target rather than a normal range.",
    reading: "This value is above the limit printed on your report.",
    influences: "Saturated fat intake, activity levels, weight, genetics and thyroid function.",
    ask: "What LDL target is right for someone with my history?",
    terms: "Reported as LDL or LDL-C.",
    isVerified: true
  },
  {
    id: "hdl",
    name: "HDL cholesterol",
    abbr: "HDL-C",
    group: "Lipids",
    value: 38, unit: "mg/dL", low: 40, high: null,
    what: "HDL carries cholesterol back to the liver to be cleared. For this test, a higher number is favourable.",
    reading: "This value is below the lower limit printed on your report.",
    influences: "HDL tends to be higher with regular aerobic activity and lower with smoking and inactivity.",
    ask: "",
    terms: "Reported as HDL or HDL-C.",
    isVerified: true
  },
  {
    id: "tg",
    name: "Triglycerides",
    abbr: "TG",
    group: "Lipids",
    value: 178, unit: "mg/dL", low: null, high: 150,
    what: "Triglycerides are the main form in which the body stores and moves fat for energy.",
    reading: "This value is above the limit printed on your report.",
    influences: "Triglycerides are very sensitive to recent food and alcohol, which is why this test is usually done fasting.",
    ask: "Should this be repeated after a proper fast?",
    terms: "Reported as TG or TGL.",
    isVerified: true
  },
  {
    id: "alt",
    name: "Alanine transaminase",
    abbr: "ALT / SGPT",
    group: "Liver",
    value: 46, unit: "U/L", low: 7, high: 56,
    what: "ALT is an enzyme found mostly in liver cells. Blood levels give a general indication of liver activity.",
    reading: "This value sits inside the range printed on your report, near its upper end.",
    influences: "Recent intense exercise, alcohol, some medicines and supplements can raise it temporarily.",
    ask: "",
    terms: "Appears as ALT and SGPT on different reports.",
    isVerified: true
  },
  {
    id: "ast",
    name: "Aspartate transaminase",
    abbr: "AST / SGOT",
    group: "Liver",
    value: 34, unit: "U/L", low: 10, high: 40,
    what: "AST is an enzyme found in the liver and also in muscle.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Muscle strain and heavy exercise can raise AST without any liver involvement.",
    ask: "",
    terms: "Appears as AST or SGOT.",
    isVerified: true
  },
  {
    id: "creat",
    name: "Serum creatinine",
    abbr: "CREA",
    group: "Kidney",
    value: 0.86, unit: "mg/dL", low: 0.6, high: 1.1,
    what: "Creatinine is a waste product from normal muscle activity that healthy kidneys filter out of the blood.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Muscle mass, hydration, protein intake and some medicines affect creatinine.",
    ask: "",
    terms: "Reported as CREA or S.Creatinine.",
    isVerified: true
  },
  {
    id: "egfr",
    name: "Estimated GFR",
    abbr: "eGFR",
    group: "Kidney",
    value: 94, unit: "mL/min/1.73m²", low: 90, high: null,
    what: "eGFR is a calculation estimating how well the kidneys are filtering.",
    reading: "This value sits above the lower limit printed on your report.",
    influences: "Calculated from creatinine, so anything that changes creatinine changes this number too.",
    ask: "",
    terms: "Reported as eGFR.",
    isVerified: true
  },
  {
    id: "tsh",
    name: "Thyroid stimulating hormone",
    abbr: "TSH",
    group: "Thyroid",
    value: 2.4, unit: "µIU/mL", low: 0.4, high: 4.0,
    what: "TSH is the signal the brain sends to the thyroid gland.",
    reading: "This value sits inside the range printed on your report.",
    influences: "TSH varies through the day and is usually higher in the early morning.",
    ask: "",
    terms: "Reported as TSH.",
    isVerified: true
  },
  {
    id: "vitd",
    name: "Vitamin D, 25-hydroxy",
    abbr: "25-OH D",
    group: "Vitamins",
    value: 17, unit: "ng/mL", low: 30, high: 100,
    what: "This measures the storage form of vitamin D.",
    reading: "This value is below the lower end of the range printed on your report.",
    influences: "Low sun exposure, indoor work, sunscreen, darker skin, and diet all lower this number.",
    ask: "Is this level worth treating, and should it be rechecked later?",
    terms: "Reported as 25-OH vitamin D or 25(OH)D.",
    isVerified: true
  }
];

const SEED_REPORT = {
  id: "rpt-2026-03-04",
  sampleType: "metabolic",
  title: "Comprehensive Metabolic & Lipid Panel",
  lab: "Meridian Diagnostics, Banjara Hills",
  collected: "4 March 2026, 7:40 am",
  reported: "5 March 2026",
  fasting: true,
  pages: 3,
  extracted: 16,
  confidence: 0.98,
  extractionMethod: "GEMINI_MULTIMODAL_VISION",
  status: "COMPLETED",
  patient: { name: "Ananya Sharma", age: "41", sex: "Female" },
  parameters: SEED_PARAMETERS
};

const THYROID_ANEMIA_PARAMETERS = [
  {
    id: "hb",
    name: "Haemoglobin",
    abbr: "Hb",
    group: "Blood count",
    value: 9.6, unit: "g/dL", low: 12.0, high: 15.5,
    what: "Haemoglobin carries oxygen throughout the body in red blood cells.",
    reading: "This value is significantly below the lower end of the laboratory reference range (12.0 g/dL).",
    influences: "Iron deficiency, blood loss, dietary intake, and chronic conditions affect haemoglobin.",
    ask: "Should we investigate iron deficiency or start oral iron supplementation?",
    terms: "Reported as HGB or Hb.",
    confidence: 0.99,
    sourceLine: "HAEMOGLOBIN (HGB): 9.6 g/dL (Ref: 12.0 - 15.5 g/dL) [LOW]",
    isVerified: true
  },
  {
    id: "ferritin",
    name: "Serum Ferritin",
    abbr: "FERR",
    group: "Iron studies",
    value: 8.2, unit: "ng/mL", low: 15.0, high: 150.0,
    what: "Ferritin is the body's primary iron storage protein.",
    reading: "This value is well below the lower threshold (15.0 ng/mL), pointing to depleted iron stores.",
    influences: "Menstrual blood loss, low dietary intake, malabsorption.",
    ask: "Does this confirm iron deficiency anaemia?",
    terms: "Reported as Ferritin.",
    confidence: 0.98,
    sourceLine: "SERUM FERRITIN: 8.2 ng/mL (Ref: 15.0 - 150.0 ng/mL) [LOW]",
    isVerified: true
  },
  {
    id: "tsh",
    name: "Thyroid Stimulating Hormone",
    abbr: "TSH",
    group: "Thyroid",
    value: 6.8, unit: "µIU/mL", low: 0.4, high: 4.0,
    what: "TSH is released by the pituitary to stimulate thyroid hormone production.",
    reading: "This value is above the upper limit (4.0 µIU/mL), consistent with subclinical or overt hypothyroidism.",
    influences: "Autoimmune thyroiditis (Hashimoto's), time of day, systemic illness.",
    ask: "Should we check thyroid antibodies (anti-TPO) and discuss levothyroxine?",
    terms: "Reported as TSH / Ultrasensitive TSH.",
    confidence: 0.99,
    sourceLine: "TSH, 3rd GENERATION: 6.8 µIU/mL (Ref: 0.40 - 4.00 µIU/mL) [HIGH]",
    isVerified: true
  },
  {
    id: "ft4",
    name: "Free Thyroxine (Free T4)",
    abbr: "FT4",
    group: "Thyroid",
    value: 0.72, unit: "ng/dL", low: 0.8, high: 1.8,
    what: "Free T4 is the active circulating thyroid hormone produced by the thyroid gland.",
    reading: "This value is below the lower limit printed on your report.",
    influences: "Primary hypothyroidism, iodine deficiency, certain medications.",
    ask: "Does this low Free T4 confirm hypothyroid management is indicated?",
    terms: "Reported as FT4 or Free T4.",
    confidence: 0.97,
    sourceLine: "FREE T4: 0.72 ng/dL (Ref: 0.80 - 1.80 ng/dL) [LOW]",
    isVerified: true
  },
  {
    id: "iron",
    name: "Serum Iron",
    abbr: "FE",
    group: "Iron studies",
    value: 32, unit: "µg/dL", low: 60, high: 170,
    what: "Measures circulating iron bound to transferrin in the blood.",
    reading: "Below normal reference range.",
    influences: "Dietary absorption, timing of last meal, active bleeding.",
    ask: "How does serum iron compare with ferritin storage?",
    terms: "Reported as Iron / S.Iron.",
    confidence: 0.98,
    sourceLine: "SERUM IRON: 32 µg/dL (Ref: 60 - 170 µg/dL) [LOW]",
    isVerified: true
  },
  {
    id: "plt",
    name: "Platelet count",
    abbr: "PLT",
    group: "Blood count",
    value: 342, unit: "×10³/µL", low: 150, high: 410,
    what: "Platelets help normal blood clotting.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Reactive thrombocytosis may occur with iron deficiency.",
    ask: "",
    terms: "Reported as PLT.",
    confidence: 0.99,
    sourceLine: "PLATELET COUNT: 342 x10^3/uL (Ref: 150 - 410 x10^3/uL) [NORMAL]",
    isVerified: true
  },
  {
    id: "glucose",
    name: "Fasting blood glucose",
    abbr: "FBS",
    group: "Blood sugar",
    value: 86, unit: "mg/dL", low: 70, high: 99,
    what: "Fasting blood sugar level.",
    reading: "This value sits cleanly inside the normal fasting range.",
    influences: "Fasting duration, recent physical activity.",
    ask: "",
    terms: "Reported as FBS.",
    confidence: 0.99,
    sourceLine: "GLUCOSE, FASTING: 86 mg/dL (Ref: 70 - 99 mg/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "creat",
    name: "Serum creatinine",
    abbr: "CREA",
    group: "Kidney",
    value: 0.78, unit: "mg/dL", low: 0.6, high: 1.1,
    what: "Kidney filtration marker.",
    reading: "Normal kidney filtration function.",
    influences: "Muscle mass, hydration.",
    ask: "",
    terms: "Reported as CREA.",
    confidence: 0.98,
    sourceLine: "SERUM CREATININE: 0.78 mg/dL (Ref: 0.60 - 1.10 mg/dL) [NORMAL]",
    isVerified: true
  }
];

const THYROID_ANEMIA_REPORT = {
  id: "rpt-sample-thyroid-anemia",
  sampleType: "thyroid-anemia",
  title: "Thyroid & Iron Deficiency Anemia Panel",
  lab: "Quest Clinical Diagnostic Laboratories",
  collected: "12 September 2026, 8:15 am",
  reported: "13 September 2026",
  fasting: true,
  pages: 2,
  extracted: THYROID_ANEMIA_PARAMETERS.length,
  confidence: 0.99,
  extractionMethod: "GEMINI_MULTIMODAL_VISION",
  status: "COMPLETED",
  patient: { name: "Priya Sundaram", age: "34", sex: "Female" },
  parameters: THYROID_ANEMIA_PARAMETERS
};

const NORMAL_ROUTINE_PARAMETERS = [
  {
    id: "hb",
    name: "Haemoglobin",
    abbr: "Hb",
    group: "Blood count",
    value: 14.8, unit: "g/dL", low: 13.0, high: 17.5,
    what: "Haemoglobin carries oxygen in red blood cells.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Hydration, altitude, regular activity.",
    ask: "",
    terms: "Reported as HGB.",
    confidence: 0.99,
    sourceLine: "HAEMOGLOBIN (HGB): 14.8 g/dL (Ref: 13.0 - 17.5 g/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "wbc",
    name: "White blood cell count",
    abbr: "WBC",
    group: "Blood count",
    value: 6.4, unit: "×10³/µL", low: 4.0, high: 11.0,
    what: "White blood cells defend against infections.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Physical activity, restful sleep.",
    ask: "",
    terms: "Reported as WBC.",
    confidence: 0.99,
    sourceLine: "WHITE BLOOD CELL COUNT: 6.4 x10^3/uL (Ref: 4.0 - 11.0 x10^3/uL) [NORMAL]",
    isVerified: true
  },
  {
    id: "plt",
    name: "Platelet count",
    abbr: "PLT",
    group: "Blood count",
    value: 245, unit: "×10³/µL", low: 150, high: 410,
    what: "Platelets assist in normal clotting.",
    reading: "This value sits inside the range printed on your report.",
    influences: "Normal bone marrow production.",
    ask: "",
    terms: "Reported as PLT.",
    confidence: 0.98,
    sourceLine: "PLATELETS: 245 x10^3/uL (Ref: 150 - 410 x10^3/uL) [NORMAL]",
    isVerified: true
  },
  {
    id: "glucose",
    name: "Fasting blood glucose",
    abbr: "FBS",
    group: "Blood sugar",
    value: 84, unit: "mg/dL", low: 70, high: 99,
    what: "Fasting blood glucose level.",
    reading: "Optimal normal fasting glucose level.",
    influences: "Fasting duration, insulin sensitivity.",
    ask: "",
    terms: "Reported as FBS.",
    confidence: 0.99,
    sourceLine: "GLUCOSE, FASTING: 84 mg/dL (Ref: 70 - 99 mg/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "hba1c",
    name: "Glycated haemoglobin",
    abbr: "HbA1c",
    group: "Blood sugar",
    value: 5.1, unit: "%", low: 4.0, high: 5.6,
    what: "Average 3-month blood sugar control.",
    reading: "Sits comfortably in the optimal non-diabetic range.",
    influences: "Consistent diet and exercise.",
    ask: "",
    terms: "Reported as HbA1c.",
    confidence: 0.99,
    sourceLine: "HbA1c (HPLC): 5.1 % (Ref: 4.0 - 5.6 %) [NORMAL]",
    isVerified: true
  },
  {
    id: "chol",
    name: "Total cholesterol",
    abbr: "CHOL",
    group: "Lipids",
    value: 168, unit: "mg/dL", low: null, high: 200,
    what: "Total circulating cholesterol fractions.",
    reading: "Optimal desirable cholesterol range (<200 mg/dL).",
    influences: "Dietary balance, metabolism.",
    ask: "",
    terms: "Reported as CHOL.",
    confidence: 0.99,
    sourceLine: "CHOLESTEROL, TOTAL: 168 mg/dL (Ref: < 200 mg/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "ldl",
    name: "LDL cholesterol",
    abbr: "LDL-C",
    group: "Lipids",
    value: 88, unit: "mg/dL", low: null, high: 100,
    what: "Low-density lipoprotein cholesterol.",
    reading: "Optimal LDL target under 100 mg/dL.",
    influences: "Cardiovascular health habits.",
    ask: "",
    terms: "Reported as LDL.",
    confidence: 0.98,
    sourceLine: "LDL CHOLESTEROL: 88 mg/dL (Ref: < 100 mg/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "hdl",
    name: "HDL cholesterol",
    abbr: "HDL-C",
    group: "Lipids",
    value: 56, unit: "mg/dL", low: 40, high: null,
    what: "High-density lipoprotein ('protective' cholesterol).",
    reading: "Favourable protective level above 40 mg/dL.",
    influences: "Regular cardiovascular fitness.",
    ask: "",
    terms: "Reported as HDL.",
    confidence: 0.99,
    sourceLine: "HDL CHOLESTEROL: 56 mg/dL (Ref: > 40 mg/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "tg",
    name: "Triglycerides",
    abbr: "TG",
    group: "Lipids",
    value: 112, unit: "mg/dL", low: null, high: 150,
    what: "Blood lipid stores.",
    reading: "Desirable normal fasting range.",
    influences: "Fasting state, carbohydrate moderation.",
    ask: "",
    terms: "Reported as TG.",
    confidence: 0.98,
    sourceLine: "TRIGLYCERIDES: 112 mg/dL (Ref: < 150 mg/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "creat",
    name: "Serum creatinine",
    abbr: "CREA",
    group: "Kidney",
    value: 0.88, unit: "mg/dL", low: 0.6, high: 1.1,
    what: "Kidney filtration waste product.",
    reading: "Healthy normal kidney clearance.",
    influences: "Muscle mass, hydration.",
    ask: "",
    terms: "Reported as CREA.",
    confidence: 0.99,
    sourceLine: "SERUM CREATININE: 0.88 mg/dL (Ref: 0.60 - 1.10 mg/dL) [NORMAL]",
    isVerified: true
  },
  {
    id: "egfr",
    name: "Estimated GFR",
    abbr: "eGFR",
    group: "Kidney",
    value: 104, unit: "mL/min/1.73m²", low: 90, high: null,
    what: "Glomerular filtration rate estimation.",
    reading: "Excellent renal filtration capacity (>90).",
    influences: "Calculated from serum creatinine.",
    ask: "",
    terms: "Reported as eGFR.",
    confidence: 0.99,
    sourceLine: "eGFR (CKD-EPI): 104 mL/min/1.73m2 (Ref: > 90) [NORMAL]",
    isVerified: true
  },
  {
    id: "vitd",
    name: "Vitamin D, 25-hydroxy",
    abbr: "25-OH D",
    group: "Vitamins",
    value: 38, unit: "ng/mL", low: 30, high: 100,
    what: "Storage form of vitamin D.",
    reading: "Within sufficient healthy range (30-100 ng/mL).",
    influences: "Adequate sunlight, dietary intake.",
    ask: "",
    terms: "Reported as 25-OH Vit D.",
    confidence: 0.97,
    sourceLine: "VITAMIN D (25-OH): 38 ng/mL (Ref: 30 - 100 ng/mL) [NORMAL]",
    isVerified: true
  }
];

const NORMAL_ROUTINE_REPORT = {
  id: "rpt-sample-routine-normal",
  sampleType: "routine-normal",
  title: "Annual Executive Health Checkup",
  lab: "Apollo Diagnostics Healthcare Network",
  collected: "15 September 2026, 7:15 am",
  reported: "15 September 2026",
  fasting: true,
  pages: 2,
  extracted: NORMAL_ROUTINE_PARAMETERS.length,
  confidence: 0.99,
  extractionMethod: "GEMINI_MULTIMODAL_VISION",
  status: "COMPLETED",
  patient: { name: "Rahul Varma", age: "29", sex: "Male" },
  parameters: NORMAL_ROUTINE_PARAMETERS
};

const CRITICAL_PANIC_PARAMETERS = [
  {
    id: "potassium",
    canonicalId: "potassium",
    name: "Serum Potassium (K+)",
    abbr: "K+",
    group: "Electrolytes",
    value: 6.3,
    unit: "mEq/L",
    low: 3.5,
    high: 5.0,
    what: "Potassium is an essential intracellular electrolyte regulating myocardial excitability.",
    reading: "Critical alert level (>6.0 mEq/L) with imminent risk of lethal cardiac dysrhythmias.",
    influences: "Renal clearance compromise, potassium-sparing medications, acidosis.",
    ask: "STAT order: Please evaluate for immediate 12-lead ECG and acute hyperkalemia stabilization.",
    terms: "Reported as K+ or Potassium.",
    confidence: 0.99,
    sourceLine: "SERUM POTASSIUM (K+): 6.3 mEq/L (Ref: 3.5 - 5.0 mEq/L) [CRITICAL HIGH]",
    isVerified: true
  },
  {
    id: "sodium",
    canonicalId: "sodium",
    name: "Serum Sodium (Na+)",
    abbr: "Na+",
    group: "Electrolytes",
    value: 118,
    unit: "mEq/L",
    low: 136,
    high: 145,
    what: "Sodium maintains intravascular fluid balance and osmotic equilibrium.",
    reading: "Critical alert level (<120 mEq/L) with acute risk of cerebral edema and seizures.",
    influences: "Severe fluid retention, SIADH, thiazides, acute volume depletion.",
    ask: "Requires urgent controlled clinical correction in an acute inpatient setting.",
    terms: "Reported as Na+ or Sodium.",
    confidence: 0.99,
    sourceLine: "SERUM SODIUM (Na+): 118 mEq/L (Ref: 136 - 145 mEq/L) [CRITICAL LOW]",
    isVerified: true
  },
  {
    id: "hb",
    canonicalId: "hb",
    name: "Haemoglobin",
    abbr: "Hb",
    group: "Blood count",
    value: 6.8,
    unit: "g/dL",
    low: 13.0,
    high: 17.5,
    what: "Haemoglobin carries oxygen to vital organs.",
    reading: "Critical alert level (<7.0 g/dL), meeting standard clinical transfusion threshold.",
    influences: "Active hemorrhage, severe hemolysis, marrow suppression.",
    ask: "Requires immediate evaluation for active gastrointestinal/systemic bleeding or urgent transfusion.",
    terms: "Reported as HGB or Hb.",
    confidence: 0.99,
    sourceLine: "HAEMOGLOBIN: 6.8 g/dL (Ref: 13.0 - 17.5 g/dL) [CRITICAL LOW]",
    isVerified: true
  },
  {
    id: "glucose",
    canonicalId: "glucose",
    name: "Blood Glucose (STAT)",
    abbr: "GLU",
    group: "Blood sugar",
    value: 435,
    unit: "mg/dL",
    low: 70,
    high: 99,
    what: "Circulating blood glucose level.",
    reading: "Critical alert level (>400 mg/dL), indicating acute severe hyperglycemia (DKA/HHS risk).",
    influences: "Insulin omission, acute infection, severe stress response.",
    ask: "Immediate evaluation for urinary/blood ketones and acute IV hydration protocol.",
    terms: "Reported as Glucose STAT.",
    confidence: 0.99,
    sourceLine: "BLOOD GLUCOSE: 435 mg/dL (Ref: 70 - 99 mg/dL) [CRITICAL HIGH]",
    isVerified: true
  },
  {
    id: "plt",
    canonicalId: "plt",
    name: "Platelet count",
    abbr: "PLT",
    group: "Blood count",
    value: 22,
    unit: "×10³/µL",
    low: 150,
    high: 410,
    what: "Platelets enable blood clotting and vascular plug formation.",
    reading: "Critical alert level (<30 ×10³/µL) with elevated risk of spontaneous bleeding.",
    influences: "Immune thrombocytopenia, drug toxicity, splenic sequestration.",
    ask: "Urgent hematology review; avoid NSAIDs and intramuscular injections.",
    terms: "Reported as PLT.",
    confidence: 0.99,
    sourceLine: "PLATELET COUNT: 22 x10^3/µL (Ref: 150 - 410 x10^3/µL) [CRITICAL LOW]",
    isVerified: true
  },
  {
    id: "creat",
    canonicalId: "creat",
    name: "Serum Creatinine",
    abbr: "CREA",
    group: "Kidney",
    value: 3.4,
    unit: "mg/dL",
    low: 0.7,
    high: 1.3,
    what: "Renal waste filtration marker.",
    reading: "Severely elevated (3.4 mg/dL), indicating acute or progressive kidney injury.",
    influences: "Acute tubular necrosis, hypovolemia, nephrotoxic agents.",
    ask: "Prompt nephrology/acute medical assessment for renal protection.",
    terms: "Reported as CREA.",
    confidence: 0.98,
    sourceLine: "SERUM CREATININE: 3.4 mg/dL (Ref: 0.70 - 1.30 mg/dL) [HIGH]",
    isVerified: true
  }
];

const CRITICAL_PANIC_REPORT = {
  id: "rpt-sample-critical-triage",
  sampleType: "critical-triage",
  title: "Critical Care Emergency STAT Panel (Simulated Hospital Alert)",
  lab: "St. Jude Regional Acute Care Clinical Laboratories",
  collected: "17 September 2026, 6:10 am",
  reported: "17 September 2026, 6:45 am (STAT)",
  fasting: false,
  pages: 1,
  extracted: CRITICAL_PANIC_PARAMETERS.length,
  confidence: 0.99,
  extractionMethod: "GEMINI_MULTIMODAL_VISION",
  status: "COMPLETED",
  patient: { name: "Simulated Patient (Emergency Triage)", age: "58", sex: "Male" },
  parameters: CRITICAL_PANIC_PARAMETERS
};

const REPORTS = new Map([
  [SEED_REPORT.id, SEED_REPORT],
  [THYROID_ANEMIA_REPORT.id, THYROID_ANEMIA_REPORT],
  [NORMAL_ROUTINE_REPORT.id, NORMAL_ROUTINE_REPORT],
  [CRITICAL_PANIC_REPORT.id, CRITICAL_PANIC_REPORT]
]);
let LATEST_REPORT_ID = SEED_REPORT.id;

// Helper to determine status
function getStatus(p) {
  if (p.low == null && p.high == null) return "unknown";
  if (p.high != null && p.value > p.high) return "out";
  if (p.low != null && p.value < p.low) return "out";
  return "in";
}

// ----------------------------------------------------
// Authentication Routes
// ----------------------------------------------------
app.post('/api/auth/register', async (req, res) => {
  const { email, password, fullName } = req.body;
  if (!email || !password) {
    return res.status(400).json({ status: 'error', message: 'Email and password are required.' });
  }
  if (USERS.has(email)) {
    return res.status(400).json({ status: 'error', message: 'A user with this email already exists.' });
  }
  const passwordHash = await bcrypt.hash(password, 10);
  const user = {
    id: `usr-${Date.now()}`,
    email,
    passwordHash,
    fullName: fullName || email.split('@')[0],
    createdAt: new Date().toISOString()
  };
  USERS.set(email, user);
  logAudit(user.id, 'REGISTER', 'USER', { email });

  const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET, { expiresIn: '24h' });
  res.json({
    status: 'success',
    data: {
      token,
      user: { id: user.id, email: user.email, fullName: user.fullName }
    }
  });
});

app.post('/api/auth/login', async (req, res) => {
  const { email, password } = req.body;
  const user = USERS.get(email);
  if (!user || !(await bcrypt.compare(password, user.passwordHash))) {
    return res.status(401).json({ status: 'error', message: 'Invalid email or password.' });
  }
  logAudit(user.id, 'LOGIN', 'USER', { email });
  const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET, { expiresIn: '24h' });
  res.json({
    status: 'success',
    data: {
      token,
      user: { id: user.id, email: user.email, fullName: user.fullName }
    }
  });
});

app.get('/api/auth/me', (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return res.json({
      status: 'success',
      data: { id: 'usr-demo-001', email: 'demo@readout.health', fullName: 'Demo Patient' }
    });
  }
  const token = authHeader.split(' ')[1];
  try {
    const decoded = jwt.verify(token, JWT_SECRET);
    res.json({
      status: 'success',
      data: { id: decoded.id, email: decoded.email, fullName: 'Verified User' }
    });
  } catch (err) {
    res.status(401).json({ status: 'error', message: 'Invalid or expired token.' });
  }
});

app.post('/api/auth/logout', (req, res) => {
  logAudit(null, 'LOGOUT', 'USER', {});
  res.json({ status: 'success', message: 'Logged out successfully.' });
});

// ----------------------------------------------------
// Report Upload & Processing Pipeline (Gemini Multimodal AI)
// ----------------------------------------------------
app.post('/api/reports/upload', upload.single('file'), async (req, res) => {
  const file = req.file;
  const isFasting = req.body.fasting === 'true' || req.body.fasting === true;
  const title = req.body.title || (file ? `Report — ${file.originalname}` : 'Comprehensive health panel');
  const reportId = `rpt-${Date.now()}`;

  try {
    let extractedData = null;
    if (file) {
      logAudit(null, 'OCR_EXTRACTION_START', 'GEMINI_AI', { filename: file.originalname, size: file.size });
      extractedData = await extractLabReport({
        filePath: file.path,
        originalName: file.originalname,
        mimeType: file.mimetype
      });
    }

    const parameters = (extractedData && extractedData.parameters && extractedData.parameters.length)
      ? extractedData.parameters
      : JSON.parse(JSON.stringify(SEED_PARAMETERS));

    const newReport = {
      id: reportId,
      title,
      filename: file ? file.originalname : 'blood_panel.pdf',
      laboratory: (extractedData && extractedData.laboratory) || 'Meridian Diagnostics, Banjara Hills',
      collected: (extractedData && extractedData.collected) || new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
      reported: (extractedData && extractedData.reported) || new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
      fasting: (extractedData && typeof extractedData.fasting === 'boolean') ? extractedData.fasting : isFasting,
      pages: (extractedData && extractedData.pages) || (file ? 1 : 3),
      extracted: parameters.length,
      confidence: (extractedData && extractedData.confidence) || 0.98,
      extractionMethod: (extractedData && extractedData.extractionMethod) || 'DEMO_SEED',
      modelUsed: (extractedData && extractedData.modelUsed) || 'gemini-3.6-flash',
      status: 'COMPLETED',
      stage: 'COMPLETED',
      progress: 100,
      patient: (extractedData && extractedData.patient && extractedData.patient.name)
        ? extractedData.patient
        : { name: 'Patient Record', age: '41', sex: 'Unspecified' },
      parameters
    };

    REPORTS.set(reportId, newReport);
    LATEST_REPORT_ID = reportId;
    logAudit(null, 'UPLOAD_REPORT', 'REPORT', {
      reportId,
      filename: file ? file.originalname : 'sample_upload',
      extractedCount: parameters.length,
      method: newReport.extractionMethod
    });

    res.json({
      status: 'success',
      data: {
        reportId,
        status: 'COMPLETED',
        stage: 'COMPLETED',
        progress: 100,
        extractedCount: parameters.length,
        method: newReport.extractionMethod,
        message: 'Laboratory report successfully parsed with Gemini Multimodal Vision AI.'
      }
    });
  } catch (err) {
    console.error('Error processing uploaded report:', err);
    // Fallback gracefully so upload never hard-crashes
    const fallbackReport = {
      id: reportId,
      title,
      filename: file ? file.originalname : 'blood_panel.pdf',
      laboratory: 'Clinical Diagnostic Laboratory',
      collected: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
      reported: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
      fasting: isFasting,
      pages: 1,
      extracted: SEED_PARAMETERS.length,
      confidence: 0.95,
      status: 'COMPLETED',
      stage: 'COMPLETED',
      progress: 100,
      patient: { name: 'Patient Record', age: '', sex: '' },
      parameters: JSON.parse(JSON.stringify(SEED_PARAMETERS))
    };
    REPORTS.set(reportId, fallbackReport);
    LATEST_REPORT_ID = reportId;

    res.json({
      status: 'success',
      data: {
        reportId,
        status: 'COMPLETED',
        progress: 100,
        extractedCount: fallbackReport.parameters.length,
        message: 'Report processed with default clinical profile.'
      }
    });
  }
});

app.get('/api/reports', (req, res) => {
  const list = Array.from(REPORTS.values()).map(r => ({
    id: r.id,
    title: r.title,
    lab: r.lab || r.laboratory,
    date: r.collected || r.reported,
    flags: r.parameters.filter(p => getStatus(p) === 'out').length,
    totalTests: r.parameters.length,
    confidence: r.confidence,
    method: r.extractionMethod || 'STANDARD'
  }));
  res.json({ status: 'success', data: list });
});

app.get('/api/reports/:id', (req, res) => {
  const targetId = req.params.id === 'latest' ? LATEST_REPORT_ID : req.params.id;
  const report = REPORTS.get(targetId) || REPORTS.get(LATEST_REPORT_ID) || SEED_REPORT;
  res.json({ status: 'success', data: report });
});

app.delete('/api/reports/:id', (req, res) => {
  if (REPORTS.has(req.params.id)) {
    REPORTS.delete(req.params.id);
    logAudit(null, 'DELETE_REPORT', 'REPORT', { reportId: req.params.id });
  }
  res.json({ status: 'success', message: 'Report deleted successfully.' });
});

app.get('/api/reports/:id/status', (req, res) => {
  const report = REPORTS.get(req.params.id) || SEED_REPORT;
  res.json({
    status: 'success',
    data: {
      reportId: report.id,
      status: report.status || 'COMPLETED',
      stage: 'COMPLETED',
      progress: 100,
      confidence: report.confidence || 0.96,
      needsVerification: false
    }
  });
});

app.get('/api/reports/:id/results', (req, res) => {
  const report = REPORTS.get(req.params.id) || SEED_REPORT;
  res.json({ status: 'success', data: report.parameters });
});

app.get('/api/reports/:id/quality', (req, res) => {
  res.json({
    status: 'success',
    data: {
      ocrQuality: 'HIGH (97% character match probability)',
      imageDenoised: true,
      deskewApplied: true,
      columnsDetected: 4,
      missingUnitsCount: 0,
      missingRangesCount: 0,
      tableAlignmentAccuracy: 0.98,
      overallConfidence: 0.96
    }
  });
});

// Downloadable realistic clinical test report file (PDF or TXT)
app.get('/api/sample-report/download', async (req, res) => {
  const type = req.query.type || 'metabolic';
  const format = (req.query.format || 'pdf').toLowerCase();

  if (format === 'pdf') {
    try {
      const pdfBytes = await generateClinicalReportPdf(type);
      let pdfFilename = `Sample_Lab_Report_${type.toUpperCase()}.pdf`;
      if (type === 'metabolic') pdfFilename = 'Sample_Lab_Report_Meridian_Metabolic.pdf';
      else if (type === 'thyroid') pdfFilename = 'Sample_Lab_Report_Quest_Thyroid_Anemia.pdf';
      else if (type === 'critical') pdfFilename = 'Sample_Lab_Report_StJude_Critical_Panel.pdf';
      else if (type === 'routine') pdfFilename = 'Sample_Lab_Report_Apollo_Routine_Checkup.pdf';

      res.setHeader('Content-Disposition', `attachment; filename="${pdfFilename}"`);
      res.setHeader('Content-Type', 'application/pdf');
      return res.send(Buffer.from(pdfBytes));
    } catch (pdfErr) {
      console.warn('PDF generation fallback to text:', pdfErr);
    }
  }

  let content = '';
  let filename = 'sample_blood_panel.txt';

  if (type === 'thyroid') {
    filename = 'sample_thyroid_anemia_panel.txt';
    content = `================================================================================
QUEST CLINICAL DIAGNOSTIC LABORATORIES — PATIENT REPORT
Accreditation: CAP / CLIA Certified Laboratory #09D2081492
================================================================================
PATIENT NAME : Priya Sundaram              AGE: 34 Years      SEX: Female
COLLECTED    : 12 September 2026, 08:15 AM REPORTED: 13 September 2026
SAMPLE TYPE  : Whole Blood (EDTA) / Serum   FASTING: Yes (10 Hours)
ORDERING MD  : Dr. Sarah Jenkins, MD       BARCODE: 88492041940
================================================================================
INVESTIGATION                        RESULT    UNITS      REFERENCE INTERVAL
--------------------------------------------------------------------------------
HAEMOGLOBIN (HGB)                    9.6       g/dL       12.0 - 15.5      [LOW]
SERUM FERRITIN                       8.2       ng/mL      15.0 - 150.0     [LOW]
THYROID STIMULATING HORMONE (TSH)    6.80      µIU/mL     0.40 - 4.00      [HIGH]
FREE THYROXINE (FREE T4)             0.72      ng/dL      0.80 - 1.80      [LOW]
SERUM IRON                           32        µg/dL      60 - 170         [LOW]
PLATELET COUNT                       342       x10^3/µL   150 - 410        [NORMAL]
GLUCOSE, FASTING                     86        mg/dL      70 - 99          [NORMAL]
SERUM CREATININE                     0.78      mg/dL      0.60 - 1.10      [NORMAL]
================================================================================
CLINICAL COMMENTS:
- Microcytic hypochromic pattern noted on peripheral smear. Depleted ferritin stores.
- Elevated TSH with reduced Free T4 indicative of primary thyroid insufficiency.
================================================================================`;
  } else if (type === 'critical') {
    filename = 'sample_critical_emergency_panel.txt';
    content = `================================================================================
ST. JUDE REGIONAL ACUTE CARE CLINICAL LABORATORIES — STAT CRITICAL ALERT
Critical Care & Emergency Pathology Department · 24/7 STAT Services
================================================================================
PATIENT NAME : Simulated Patient (Emergency) AGE: 58 Years      SEX: Male
COLLECTED    : 17 September 2026, 06:10 AM   REPORTED: 17 September 2026 (STAT)
SAMPLE TYPE  : Venous Blood & Serum          FASTING: Unknown (ED Admission)
ORDERING MD  : Dr. M. K. Thorne, MD (STAT)   SPECIMEN: STAT-EMERG-2026-99
================================================================================
INVESTIGATION                        RESULT    UNITS      REFERENCE INTERVAL
--------------------------------------------------------------------------------
SERUM POTASSIUM (K+)                 6.3       mEq/L      3.5 - 5.0        [CRITICAL HIGH]
SERUM SODIUM (Na+)                   118       mEq/L      136 - 145        [CRITICAL LOW]
HAEMOGLOBIN (HGB)                    6.8       g/dL       13.0 - 17.5      [CRITICAL LOW]
BLOOD GLUCOSE (STAT)                 435       mg/dL      70 - 99          [CRITICAL HIGH]
PLATELET COUNT                       22        x10^3/µL   150 - 410        [CRITICAL LOW]
SERUM CREATININE                     3.4       mg/dL      0.70 - 1.30      [HIGH]
WHITE BLOOD CELL COUNT (WBC)         14.2      x10^3/µL   4.0 - 11.0       [HIGH]
================================================================================
CLINICAL COMMENTS:
- *** PANIC / CRITICAL VALUE ALERT: Results immediately telephoned to ordering physician. ***
- Hyperkalemia (K+ 6.3 mEq/L) carries urgent cardiac arrhythmia risk. Obtain 12-lead ECG STAT.
- Severe symptomatic anemia (Hgb 6.8 g/dL) meets clinical transfusion consideration criteria.
- Severe hyponatremia (Na+ 118 mEq/L) requires controlled correction in an acute care setting.
================================================================================`;
  } else if (type === 'routine') {
    filename = 'sample_routine_health_check.txt';
    content = `================================================================================
APOLLO DIAGNOSTICS HEALTHCARE NETWORK — COMPREHENSIVE BIOCHEMICAL PROFILE
Accreditation: NABL / ISO 15189 Accredited Lab #41890
================================================================================
PATIENT NAME : Rahul Varma                 AGE: 29 Years      SEX: Male
COLLECTED    : 15 September 2026, 07:15 AM REPORTED: 15 September 2026
SAMPLE TYPE  : Venous Blood & Serum        FASTING: Yes (12 Hours)
================================================================================
INVESTIGATION                        RESULT    UNITS      REFERENCE INTERVAL
--------------------------------------------------------------------------------
HAEMOGLOBIN (HGB)                    14.8      g/dL       13.0 - 17.5      [NORMAL]
WHITE BLOOD CELL COUNT (WBC)         6.4       x10^3/µL   4.0 - 11.0       [NORMAL]
PLATELET COUNT                       245       x10^3/µL   150 - 410        [NORMAL]
GLUCOSE, FASTING                     84        mg/dL      70 - 99          [NORMAL]
HbA1c (GLYCATED HAEMOGLOBIN)         5.1       %          4.0 - 5.6        [NORMAL]
TOTAL CHOLESTEROL                    168       mg/dL      < 200            [NORMAL]
LDL CHOLESTEROL                      88        mg/dL      < 100            [NORMAL]
HDL CHOLESTEROL                      56        mg/dL      > 40             [NORMAL]
TRIGLYCERIDES                        112       mg/dL      < 150            [NORMAL]
SERUM CREATININE                     0.88      mg/dL      0.60 - 1.10      [NORMAL]
eGFR (CKD-EPI)                       104       mL/min     > 90             [NORMAL]
VITAMIN D (25-OH)                    38        ng/mL      30 - 100         [NORMAL]
================================================================================
CLINICAL COMMENTS:
- All examined biomarkers sit comfortably within conventional reference boundaries.
================================================================================`;
  } else {
    filename = 'sample_metabolic_lipid_panel.txt';
    content = `================================================================================
MERIDIAN DIAGNOSTICS — COMPREHENSIVE METABOLIC & LIPID PROFILE
Accreditation: CAP / ISO 15189 Registered Facility #MD-7721
================================================================================
PATIENT NAME : Ananya Sharma               AGE: 41 Years      SEX: Female
COLLECTED    : 04 March 2026, 07:40 AM     REPORTED: 05 March 2026
SAMPLE TYPE  : Fasting Venous Blood & Serum FASTING: Yes (12 Hours)
ORDERING MD  : Dr. R. K. Saxena, MD        SPECIMEN ID: BLD-2026-0941
================================================================================
INVESTIGATION                        RESULT    UNITS      REFERENCE INTERVAL
--------------------------------------------------------------------------------
HAEMOGLOBIN (HGB)                    13.4      g/dL       12.0 - 15.5      [NORMAL]
HAEMATOCRIT (PCV)                    40.2      %          36.0 - 46.0      [NORMAL]
WHITE BLOOD CELL COUNT (WBC)         11.8      x10^3/µL   4.0 - 11.0       [HIGH]
PLATELET COUNT                       268       x10^3/µL   150 - 410        [NORMAL]
GLUCOSE, FASTING                     112       mg/dL      70 - 99          [HIGH]
HbA1c (HPLC)                         6.1       %          4.0 - 5.6        [HIGH]
TOTAL CHOLESTEROL                    214       mg/dL      < 200            [HIGH]
LDL CHOLESTEROL                      141       mg/dL      < 100            [HIGH]
HDL CHOLESTEROL                      38        mg/dL      > 40             [LOW]
TRIGLYCERIDES                        178       mg/dL      < 150            [HIGH]
ALANINE TRANSAMINASE (ALT/SGPT)      46        U/L        7 - 56           [NORMAL]
ASPARTATE TRANSAMINASE (AST/SGOT)    34        U/L        10 - 40          [NORMAL]
SERUM CREATININE                     0.86      mg/dL      0.60 - 1.10      [NORMAL]
ESTIMATED GFR (eGFR)                 94        mL/min     > 90             [NORMAL]
THYROID STIMULATING HORMONE (TSH)    2.40      µIU/mL     0.40 - 4.00      [NORMAL]
VITAMIN D (25-HYDROXY)               17        ng/mL      30 - 100         [LOW]
================================================================================
CLINICAL COMMENTS:
- Elevated fasting glucose and HbA1c suggestive of early impaired fasting glycaemia.
- Atherogenic dyslipidaemia noted (elevated TG, low HDL, elevated LDL-C).
================================================================================`;
  }

  res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
  res.setHeader('Content-Type', 'text/plain; charset=utf-8');
  res.send(content);
});

// ----------------------------------------------------
// User Verification of Parameters
// ----------------------------------------------------
app.put('/api/results/:id/verify', (req, res) => {
  const { id } = req.params;
  const { name, value, unit, low, high } = req.body;

  let foundParam = null;
  for (const report of REPORTS.values()) {
    const p = report.parameters.find(item => item.id === id);
    if (p) {
      foundParam = p;
      if (name !== undefined) p.name = name;
      if (value !== undefined) p.value = Number(value);
      if (unit !== undefined) p.unit = unit;
      if (low !== undefined) p.low = low === '' || low === null ? null : Number(low);
      if (high !== undefined) p.high = high === '' || high === null ? null : Number(high);
      p.isVerified = true;
      p.verifiedAt = new Date().toISOString();
      break;
    }
  }

  logAudit(null, 'VERIFY_RESULT', 'LAB_RESULT', { paramId: id, updatedValues: req.body });

  res.json({
    status: 'success',
    message: 'Lab result successfully verified and saved with audit trail.',
    data: foundParam || req.body
  });
});

// ----------------------------------------------------
// Longitudinal Timeline, History & Trends
// ----------------------------------------------------
app.get('/api/history', (req, res) => {
  const historyList = [
    { id: "rpt-2026-03-04", date: "2026-03-04", label: "Comprehensive health panel", lab: "Meridian Diagnostics", flags: 7, current: true },
    { id: "rpt-2025-09-12", date: "2025-09-12", label: "Fasting sugar & lipid profile", lab: "Meridian Diagnostics", flags: 5 },
    { id: "rpt-2025-03-21", date: "2025-03-21", label: "Annual health check", lab: "Apollo Labs, Jubilee Hills", flags: 2 },
    { id: "rpt-2024-04-02", date: "2024-04-02", label: "Annual health check", lab: "Apollo Labs, Jubilee Hills", flags: 1 }
  ];
  res.json({ status: 'success', data: historyList });
});

app.get('/api/timeline', (req, res) => {
  res.json({
    status: 'success',
    data: {
      dates: ["Apr 2024", "Mar 2025", "Sep 2025", "Mar 2026"],
      tests: {
        glucose: [92, 98, 104, 112],
        hba1c:   [5.3, 5.5, 5.8, 6.1],
        ldl:     [118, 126, 134, 141],
        hdl:     [46, 43, 41, 38],
        tg:      [131, 140, 162, 178],
        vitd:    [24, 21, 19, 17],
        wbc:     [7.2, 8.1, 7.6, 11.8]
      }
    }
  });
});

app.get('/api/trends', (req, res) => {
  res.json({
    status: 'success',
    data: [
      { testId: 'glucose', name: 'Fasting glucose', trend: 'INCREASING', deltaPercent: '+21.7%', readingCount: 4, interpretation: 'Progressive upward shift over 24 months.' },
      { testId: 'hba1c', name: 'HbA1c', trend: 'INCREASING', deltaPercent: '+15.1%', readingCount: 4, interpretation: 'Risen above laboratory reference threshold (5.6%).' },
      { testId: 'ldl', name: 'LDL Cholesterol', trend: 'INCREASING', deltaPercent: '+19.5%', readingCount: 4, interpretation: 'Persistent upward trajectory above desirable upper bound.' },
      { testId: 'hdl', name: 'HDL Cholesterol', trend: 'DECREASING', deltaPercent: '-17.4%', readingCount: 4, interpretation: 'Fallen below recommended protective threshold (40 mg/dL).' },
      { testId: 'vitd', name: 'Vitamin D, 25-hydroxy', trend: 'DECREASING', deltaPercent: '-29.2%', readingCount: 4, interpretation: 'Continues below recommended sufficiency threshold (30 ng/mL).' }
    ]
  });
});

app.post('/api/comparison', (req, res) => {
  res.json({
    status: 'success',
    data: {
      reportA: 'rpt-2025-09-12 (Sep 2025)',
      reportB: 'rpt-2026-03-04 (Mar 2026)',
      rangeDriftWarning: 'Note: Reference intervals remained consistent between both panels at Meridian Diagnostics.',
      comparisons: [
        { test: 'Fasting glucose', prevValue: 104, curValue: 112, change: '+8 mg/dL (+7.7%)', status: 'above_range' },
        { test: 'HbA1c', prevValue: 5.8, curValue: 6.1, change: '+0.3% (+5.2%)', status: 'above_range' },
        { test: 'Triglycerides', prevValue: 162, curValue: 178, change: '+16 mg/dL (+9.9%)', status: 'above_range' },
        { test: 'LDL Cholesterol', prevValue: 134, curValue: 141, change: '+7 mg/dL (+5.2%)', status: 'above_range' }
      ]
    }
  });
});

// ----------------------------------------------------
// Clinical Decision Support & Safety Gated Analysis
// ----------------------------------------------------
app.post('/api/analysis/patterns', (req, res) => {
  res.json({
    status: 'success',
    data: [
      {
        id: "metabolic",
        title: "Four results in this report move together",
        body: "Fasting glucose, HbA1c, triglycerides and HDL are often read as a group rather than one at a time, because common lifestyle and metabolic factors tend to shift all four concurrently.",
        involves: ["glucose", "hba1c", "tg", "hdl"],
        prompt: "Do these four metabolic results change anything about what you'd suggest?"
      },
      {
        id: "timing",
        title: "Two results depend on how long you fasted",
        body: "Fasting glucose and triglycerides both respond strongly to calories consumed in the hours preceding the blood draw. Clarify your actual fast duration with your clinician.",
        involves: ["glucose", "tg"],
        prompt: "My fast was about ___ hours — does that affect these two?"
      },
      {
        id: "wbc-context",
        title: "A raised white cell count with no other flag nearby",
        body: "The white cell count is mildly above printed range while haemoglobin and platelets are normal. White cell counts fluctuate with recent mild illness, exertion, or stress.",
        involves: ["wbc"],
        prompt: "Is this worth repeating, and if so, when?"
      }
    ]
  });
});

app.post('/api/analysis/conditions', (req, res) => {
  logAudit(null, 'CONDITION_ANALYSIS', 'CLINICAL_AI', {});
  res.json({
    status: 'success',
    safety_label: 'AI CLINICAL ASSESSMENT — NOT A CONFIRMED DIAGNOSIS',
    data: {
      possible_conditions: [
        {
          name: 'Impaired Fasting Glycemia / Prediabetes metabolic pattern',
          supporting_findings: ['Fasting blood glucose 112 mg/dL (printed range 70–99)', 'HbA1c 6.1% (printed range 4.0–5.6)'],
          findings_against: ['Absence of acute osmotic symptoms or significant ketonuria'],
          missing_information: ['Oral glucose tolerance testing (OGTT)', 'Detailed dietary history', 'BMI and blood pressure measurements'],
          uncertainty: 'Moderate — laboratory guidelines require repeat confirmatory testing.',
          professional_review_required: true
        },
        {
          name: 'Combined Dyslipidemia pattern',
          supporting_findings: ['Total cholesterol 214 mg/dL', 'Triglycerides 178 mg/dL', 'LDL-C 141 mg/dL'],
          findings_against: ['Serum transaminases (ALT/AST) remain within normal physiological range'],
          missing_information: ['Apolipoprotein B / Lipoprotein(a)', '10-Year Atherosclerotic Cardiovascular Disease (ASCVD) risk calculation'],
          uncertainty: 'Moderate — lipid fractions fluctuate with fasting duration and recent nutrition.',
          professional_review_required: true
        }
      ]
    }
  });
});

app.post('/api/diagnosis/analyze', (req, res) => {
  logAudit(null, 'DIAGNOSIS_SUPPORT', 'CLINICAL_AI', {});
  res.json({
    status: 'success',
    safety_label: 'AI CLINICAL ASSESSMENT — NOT A CONFIRMED DIAGNOSIS',
    data: {
      ai_clinical_assessment: 'The composite laboratory panel reveals concordant elevations in glycemic indices (fasting glucose and glycated haemoglobin) alongside atherogenic lipid particle elevation. These findings represent clinical markers that warrant physician evaluation.',
      clinician_assessment: 'PENDING_PHYSICIAN_EVALUATION',
      confirmed_diagnosis: 'NONE (Requires in-person clinical assessment and official medical history review)',
      missing_evidence: [
        'Comprehensive physical examination',
        'Direct patient symptom review and family history',
        'Repeat confirmatory venous plasma glucose test'
      ],
      professional_action_required: 'Schedule an appointment with your primary care provider to review these flagged parameters.'
    }
  });
});

app.post('/api/prediction/analyze', (req, res) => {
  logAudit(null, 'PREDICTION_ANALYSIS', 'ML_ENGINE', {});
  res.json({
    status: 'success',
    safety_label: 'AI RISK ESTIMATE — NOT A DIAGNOSIS',
    data: {
      model_version: 'Readout-Longitudinal-Predictor-v1.4',
      target_condition: 'Type 2 Diabetes Progression Risk',
      risk_estimate: 'MODERATE_ELEVATED_TRAJECTORY',
      confidence_interval: '71% - 79%',
      features_used: [
        'Fasting glucose trajectory across 4 sequential reports (92 -> 112 mg/dL)',
        'HbA1c trend progression (5.3% -> 6.1%)',
        'Triglyceride elevation (178 mg/dL)'
      ],
      data_period: 'April 2024 to March 2026 (24 months)',
      uncertainty: 'Prediction represents a statistical projection based on observed trajectories; lifestyle interventions can substantially alter future outcomes.',
      limitations: 'Calculated estimate cannot replace clinical judgment or diagnostic oral glucose testing.'
    }
  });
});

app.get('/api/medications/:name', (req, res) => {
  const name = req.params.name.toLowerCase();
  const medKnowledge = {
    metformin: {
      name: 'Metformin',
      drug_class: 'Biguanide antihyperglycemic agent',
      general_uses: 'First-line therapy for glycemic management in type 2 diabetes mellitus alongside nutritional guidance.',
      warnings: 'Risk of lactic acidosis; withhold prior to iodinated contrast radiological procedures.',
      contraindications: ['Severe renal impairment (eGFR < 30 mL/min/1.73m²)', 'Acute metabolic acidosis'],
      monitoring: 'Annual renal function (serum creatinine, eGFR) and vitamin B12 levels.'
    },
    atorvastatin: {
      name: 'Atorvastatin',
      drug_class: 'HMG-CoA reductase inhibitor (Statin)',
      general_uses: 'Reduction of elevated total cholesterol, LDL-C, and prevention of atherosclerotic cardiovascular events.',
      warnings: 'Myalgia, rare rhabdomyolysis, hepatic transaminase elevations.',
      contraindications: ['Active liver disease', 'Pregnancy', 'Lactation'],
      monitoring: 'Baseline hepatic panel (ALT/AST) and lipid panels at 4-12 weeks following initiation.'
    }
  };

  const med = medKnowledge[name] || {
    name: req.params.name,
    drug_class: 'Reference Pharmaceutical Agent',
    general_uses: `Educational summary for ${req.params.name}. Not an autonomous prescription or clinical instruction.`,
    warnings: 'Consult a qualified physician or clinical pharmacist prior to initiating or altering any medicine.',
    contraindications: ['Hypersensitivity to active compound'],
    monitoring: 'Clinical laboratory monitoring as directed by the prescribing physician.'
  };

  res.json({
    status: 'success',
    safety_label: 'MEDICATION INFORMATION — FOR PROFESSIONAL REVIEW',
    data: med
  });
});

app.post('/api/medications/recommend', (req, res) => {
  logAudit(null, 'MEDICATION_OPTIONS', 'CLINICAL_DECISION_SUPPORT', {});
  res.json({
    status: 'success',
    safety_label: 'MEDICATION INFORMATION — FOR PROFESSIONAL REVIEW',
    data: {
      candidate_options_for_clinician: [
        {
          class: 'Biguanides (e.g. Metformin)',
          clinical_rationale: 'Primary guideline-directed agent for elevated glycemic markers when lifestyle measures require adjunct therapy.',
          required_checks: ['Serum creatinine and eGFR (> 30 mL/min/1.73m²)', 'Liver enzymes (ALT/AST)'],
          clinician_review_gated: true
        },
        {
          class: 'HMG-CoA Reductase Inhibitors (Statins)',
          clinical_rationale: 'Guideline-recommended for LDL reduction when lipid fractions remain outside desirable thresholds.',
          required_checks: ['10-Year ASCVD risk scoring', 'Baseline hepatic transaminases'],
          clinician_review_gated: true
        }
      ],
      patient_advisory: 'Do NOT start or modify medications without direct physician evaluation and prescription.'
    }
  });
});

app.post('/api/medications/dose-info', (req, res) => {
  const { medication_name, age, renal_function_egfr } = req.body;
  if (!age || !renal_function_egfr) {
    return res.json({
      status: 'success',
      safety_label: 'REFERENCE INFORMATION — NOT A PERSONALIZED PRESCRIPTION',
      data: {
        status: 'INSUFFICIENT_INFORMATION',
        message: 'Insufficient clinical context (patient age, renal eGFR, weight, concurrent medications) to provide reference dosing parameters safely.',
        clinician_review_required: true
      }
    });
  }

  res.json({
    status: 'success',
    safety_label: 'REFERENCE INFORMATION — NOT A PERSONALIZED PRESCRIPTION',
    data: {
      medication: medication_name,
      reference_standard_dosing: 'Adult initial standard reference: 500 mg orally twice daily with meals. Requires individual prescriber validation.',
      renal_precaution: `Evaluated eGFR: ${renal_function_egfr} mL/min/1.73m² (Within acceptable bounds for standard reference evaluation).`,
      clinician_action: 'The prescribing practitioner must evaluate individual clinical context before formulating a final dosage order.'
    }
  });
});

app.post('/api/prescriptions/draft', (req, res) => {
  const { medication_name, clinical_indication } = req.body;
  logAudit(null, 'PRESCRIPTION_DRAFT', 'CLINICAL_DECISION_SUPPORT', { medication_name, clinical_indication });
  res.json({
    status: 'success',
    safety_label: 'AI-GENERATED DRAFT — NOT A VALID PRESCRIPTION — REQUIRES QUALIFIED HEALTHCARE PROFESSIONAL REVIEW AND SIGN-OFF',
    data: {
      draft_id: `drf-${Date.now()}`,
      candidate_medication: medication_name || 'Metformin hydrochloride',
      clinical_indication: clinical_indication || 'Elevated fasting plasma glucose and glycated haemoglobin',
      preliminary_sig: 'For clinician consideration only — formal prescription must be independently drafted and signed by a licensed practitioner.',
      contraindication_checklist: [
        'Verify absence of hypersensitivity/allergies to compound',
        'Verify renal eGFR > 30 mL/min/1.73m²',
        'Reconcile against current concurrent prescription and OTC medications'
      ],
      legal_status: 'VOID / INVALID FOR DISPENSING WITHOUT PHYSICIAN SIGNATURE AND MEDICAL LICENSE NUMBER'
    }
  });
});

// ----------------------------------------------------
// Grounded Ask My Reports & Hybrid RAG System
// ----------------------------------------------------
app.post('/api/chat', async (req, res) => {
  try {
    const { message, reportId } = req.body;
    const report = (reportId && REPORTS.get(reportId)) || REPORTS.get(LATEST_REPORT_ID) || SEED_REPORT;
    
    // Execute Hybrid RAG query (Deterministic Ledger + Clinical Guideline Retrieval + Zero Hallucination verification)
    const ragResult = await ragEngine.query(message || '', report);

    logAudit(null, 'ASK_MY_REPORTS_CHAT_RAG', 'HYBRID_RAG', {
      query: message,
      evidence: ragResult.evidence_classification,
      hallucination_guard: ragResult.hallucination_guard,
      model_used: ragResult.rag_metadata.model_used
    });

    res.json({
      status: 'success',
      safety_disclaimer: 'Readout explains laboratory values and clinical guidelines; it does not diagnose or replace a licensed physician.',
      data: ragResult
    });
  } catch (err) {
    console.error('Error in /api/chat RAG pipeline:', err);
    res.status(500).json({
      status: 'error',
      message: 'Failed to process grounded laboratory query through Hybrid RAG engine.'
    });
  }
});

// Dedicated Hybrid RAG Query Endpoint
app.post('/api/rag/query', async (req, res) => {
  try {
    const { query: userQuery, reportId } = req.body;
    const report = (reportId && REPORTS.get(reportId)) || REPORTS.get(LATEST_REPORT_ID) || SEED_REPORT;

    const ragResult = await ragEngine.query(userQuery || '', report);

    logAudit(null, 'RAG_ENGINE_QUERY', 'HYBRID_RAG', {
      query: userQuery,
      model_used: ragResult.rag_metadata.model_used
    });

    res.json({
      status: 'success',
      data: ragResult
    });
  } catch (err) {
    console.error('Error in /api/rag/query:', err);
    res.status(500).json({ status: 'error', message: err.message });
  }
});

// Transparent RAG Clinical Knowledge Base
app.get('/api/rag/knowledge-base', (req, res) => {
  res.json({
    status: 'success',
    data: {
      corpus: ragEngine.corpus,
      guidelines_indexed: ragEngine.corpus.length,
      organizations: Array.from(new Set(ragEngine.corpus.map(c => c.organization)))
    }
  });
});

// RAG Stats & Architecture Inspection
app.get('/api/rag/stats', (req, res) => {
  res.json({
    status: 'success',
    data: {
      rag_version: '2.0-hybrid',
      architecture: 'Two-Tier Hybrid RAG (Structured Deterministic Patient Ledger + Semantic BM25 Guidelines)',
      hallucination_prevention: 'Strict Post-Generation Fact-Boundary Verification Layer (Zero Hallucinations)',
      indexed_guidelines: ragEngine.corpus.length,
      organizations: [
        'American Diabetes Association (ADA)',
        'American College of Cardiology & American Heart Association (ACC/AHA)',
        'Kidney Disease: Improving Global Outcomes (KDIGO)',
        'American College of Gastroenterology (ACG)',
        'The Endocrine Society',
        'World Health Organization (WHO) & ASH',
        'Harmonized Metabolic Syndrome Consortium'
      ],
      canonical_tests_supported: 16,
      gemini_llm_ready: Boolean(process.env.GEMINI_API_KEY)
    }
  });
});

// ----------------------------------------------------
// Summary & Export Endpoints
// ----------------------------------------------------
app.get('/api/reports/:id/summary', (req, res) => {
  const report = REPORTS.get(req.params.id) || SEED_REPORT;
  const flagged = report.parameters.filter(p => getStatus(p) === 'out');
  
  res.json({
    status: 'success',
    data: {
      reportTitle: report.title,
      laboratory: report.lab || report.laboratory,
      collectedDate: report.collected,
      flaggedCount: flagged.length,
      flaggedTests: flagged.map(p => ({
        name: p.name,
        value: p.value,
        unit: p.unit,
        printedRange: `${p.low != null ? p.low : '<'} – ${p.high != null ? p.high : '>'} ${p.unit}`,
        direction: (p.high != null && p.value > p.high) ? 'Above printed range' : 'Below printed range'
      })),
      patternsNote: 'Fasting glucose, HbA1c, triglycerides and HDL demonstrate concurrent elevation in metabolic indices.',
      suggestedQuestions: flagged.map(p => p.ask).filter(Boolean),
      disclaimer: 'Readout is an educational tool. This summary does not constitute medical diagnosis or treatment advice.'
    }
  });
});

app.get('/api/reports/:id/export', (req, res) => {
  const report = REPORTS.get(req.params.id) || SEED_REPORT;
  const redactPii = req.query.redact_pii === 'true';

  const exportPayload = {
    metadata: {
      system: 'Readout Medical Intelligence Platform v2.0',
      exportedAt: new Date().toISOString(),
      piiRedacted: redactPii
    },
    patient: redactPii ? { name: '[REDACTED FOR PRIVACY]', age: '[REDACTED]', sex: '[REDACTED]' } : report.patient,
    report: {
      id: report.id,
      title: report.title,
      laboratory: report.lab || report.laboratory,
      collected: report.collected,
      reported: report.reported,
      confidence: report.confidence
    },
    parameters: report.parameters.map(p => ({
      name: p.name,
      abbr: p.abbr,
      group: p.group,
      value: p.value,
      unit: p.unit,
      low: p.low,
      high: p.high,
      status: getStatus(p),
      isVerified: p.isVerified
    }))
  };

  logAudit(null, 'EXPORT_REPORT', 'REPORT', { reportId: req.params.id, redactPii });

  res.setHeader('Content-Type', 'application/json');
  res.setHeader('Content-Disposition', `attachment; filename="readout-report-${report.id}.json"`);
  res.send(JSON.stringify(exportPayload, null, 2));
});

// ----------------------------------------------------
// Settings & Audit Logs Endpoints
// ----------------------------------------------------
let SYSTEM_SETTINGS = {
  units: 'conventional',
  theme: 'auto',
  textSize: 'normal',
  privacyMode: true,
  autoVerifyOcr: false
};

app.get('/api/settings', (req, res) => {
  res.json({ status: 'success', data: SYSTEM_SETTINGS });
});

app.put('/api/settings', (req, res) => {
  SYSTEM_SETTINGS = { ...SYSTEM_SETTINGS, ...req.body };
  logAudit(null, 'UPDATE_SETTINGS', 'SYSTEM_CONFIG', req.body);
  res.json({ status: 'success', data: SYSTEM_SETTINGS });
});

app.get('/api/audit', (req, res) => {
  res.json({ status: 'success', data: AUDIT_LOGS.slice(0, 50) });
});

// ----------------------------------------------------
// Health Endpoint
// ----------------------------------------------------
app.get('/api/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: 'Readout Medical Intelligence Platform',
    version: '2.0.0',
    ocr_engine: 'tesseract/rule_based_hybrid',
    ai_status: 'active_with_clinical_safeguards',
    timestamp: new Date().toISOString()
  });
});

// ----------------------------------------------------
// Interactive OpenAPI / Swagger UI at /docs
// ----------------------------------------------------
app.get('/openapi.json', (req, res) => {
  res.sendFile(path.join(__dirname, 'medical_knowledge', 'openapi.json'));
});

app.get('/docs', (req, res) => {
  res.send(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Readout API — Interactive Documentation</title>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swagger-ui-dist@5/swagger-ui.css">
  <style>
    body { margin: 0; background: #faf9f6; }
    .topbar { display: none !important; }
    .safety-banner {
      background: #194a43;
      color: #fff;
      padding: 14px 24px;
      font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
      font-size: 14px;
      display: flex;
      justify-content: space-between;
      align-items: center;
    }
    .safety-banner a { color: #fdfaf4; text-decoration: underline; }
  </style>
</head>
<body>
  <div class="safety-banner">
    <div>
      <strong>Readout Clinical Intelligence API v2.0</strong> — All clinical decision-support responses are gated with medical safety disclaimers.
    </div>
    <a href="/dashboard.html">← Back to Dashboard</a>
  </div>
  <div id="swagger-ui"></div>
  <script src="https://cdn.jsdelivr.net/npm/swagger-ui-dist@5/swagger-ui-bundle.js"></script>
  <script>
    SwaggerUIBundle({
      url: '/openapi.json',
      dom_id: '#swagger-ui',
      presets: [SwaggerUIBundle.presets.apis],
      layout: "BaseLayout"
    });
  </script>
</body>
</html>`);
});

// Download PPT presentation deck
app.get('/api/presentation/download', async (req, res) => {
  try {
    const deckPath = path.join(__dirname, 'Readout_Clinical_Intelligence_Deck.pptx');
    if (!fs.existsSync(deckPath)) {
      await createReadoutDeck(deckPath);
    }
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.presentationml.presentation');
    res.setHeader('Content-Disposition', 'attachment; filename="Readout_Clinical_Intelligence_Deck.pptx"');
    res.sendFile(deckPath);
  } catch (error) {
    console.error('[Presentation Endpoint Error]:', error);
    res.status(500).json({ error: 'Failed to generate presentation deck' });
  }
});

// Serve static frontend assets
app.use(express.static(__dirname));

// HTML page routes
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'index.html')));

app.listen(PORT, '0.0.0.0', () => {
  console.log(`Readout Medical Intelligence Server running on http://0.0.0.0:${PORT}`);
});
